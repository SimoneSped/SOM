...
def generate_example_spectra(probabilities, size_spectra, max_components, snr,separation, desired_features = None):
    ...
    for i in range((len(spectra))):
        spectra[i].intensities = (spectra[i].intensities - np.amin(list_min)) / (np.amax(list_max) - np.amin(list_min))
    ...
    for i in range(1, columns*rows+1):
        ...
        plt.plot(
            spectra[i-1].lambda_range, spectra[i-1].intensities,
            c='black',
            alpha=0.5,
...
def visualize_neurons_map(SOM, best_n_to_highlight = 5):
    ...
    plt.title(
        "Resulting neuron map with " + str(len(SOM.clusters)) + " found clusters, best " + str(
        best_n_to_highlight) + " of them"
    )
    plt.xlabel("x")
    plt.ylabel("y")
    plt.show()
...
def visualize_best_n_clusters(SOM, number_of_clusters, number_of_spectra):
    ...
    for i in range(0, number_of_clusters):
        ...
        df = SOM.matches_input_to_clusters.loc[SOM.matches_input_to_clusters['Cluster_number'] == i]
        for j in range(0, number_of_spectra):
            plt.plot(x, input_data[df.iloc[j].Index], c='black', alpha=0.5)
            plt.xlim([0, len(x)])
            
    fig.suptitle("Best matches for the best "+str(number_of_clusters)+" clusters (out of a total of "+str(len(SOM.clusters))+" clusters)")
    fig.supxlabel("Channels")
    fig.supylabel("Intensity")
    plt.tight_layout()
    plt.show()
    ...
    if len(df) >= num_pages*(columns*rows):
        for i in range(0, num_pages):
            ...
            for j in range(0, columns*rows):
                ...
                plt.plot(x, input_data[df.iloc[i*(columns*rows) + j].Index], c='black', alpha=0.5)   
                plt.plot(x, df_averaged_spectra['Avg_Spectrum'][cluster_number], c='red', alpha=0.5)
                plt.xlim([0, len(x)])
            fig.suptitle("Representative matches and averaged spectrum of cluster no."+str(cluster_number+1)+" (out of a total of "+str(len(df))+" spectra)")
            fig.supxlabel("Channels")
            fig.supylabel("Intensity")
            plt.tight_layout()
            plt.show()
        ...
        for k in range(0, len(df)-rest*(columns*rows)):
            ...
            plt.plot(x, input_data[df.iloc[rest*(columns*rows) + k].Index], c='black', alpha=0.5)  
            plt.plot(x, df_averaged_spectra['Avg_Spectrum'][cluster_number], c='red', alpha=0.5)
            plt.xlim([0, len(x)])
        fig.suptitle("Representative matches and averaged spectrum of cluster no."+str(cluster_number+1)+" (out of a total of "+str(len(df))+" spectra)")
        fig.supxlabel("Channels")
...
def fit_clusters(SOM, df_fit, input_data):
    ...
    for cluster_number in range(0, to_fit_clusters):
        ...
        number_guessed_components = int(input("-- Cluster "+str(cluster_number+1)+" -- Insert the guess for the number of components: "))
        ...
        if number_guessed_components > 0:
            ...
            for i in range(0, number_guessed_components):
                print("")
                print("===============================================")
                print("-- Guesses for the component no. "+str(i+1)+"/"+str(number_guessed_components)+" --")
                print("===============================================")
            ...
            for i in range(0, number_guessed_components):
                gg_init = np.append(gg_init, models.Gaussian1D(
            ...
            for i in range(0, len(df)):
                ...
                df_fit = df_fit.append(
                    pd.DataFrame(
                        [[cluster_number, gg_fit, df.iloc[i].Index, calculate_data_model_deviation(data, gg_fit(x))]],
...
def visualize_fits(SOM, df_fit, cluster_number, num_pages):
    ...
    if len(df_fit) >= num_pages*(columns*rows):
        for i in range(0, num_pages):
            ...
            for j in range(0, columns*rows):
                fig.add_subplot(rows, columns, j+1)
                plt.plot(x, input_data[df_fit.iloc[i*(columns*rows) + j].Index_spectrum], c='black', alpha=0.5)
                plt.plot(x, df_fit.iloc[i*(columns*rows) + j].fitter(x), c='orange', alpha= 0.6)
                plt.xlim([0, len(x)])
            fig.suptitle("Fits for the cluster no."+str(cluster_number)+" (out of a total of "+str(len(df_fit))+" spectra)")
            fig.supxlabel("Channels")
            fig.supylabel("Intensity")
            plt.tight_layout()
            plt.show()
        ...
        for i in range(0, rest):
            ...
            for j in range(0, columns*rows):
                fig.add_subplot(rows, columns, j+1)
                plt.plot(x, input_data[df_fit.iloc[i*(columns*rows) + j].Index], c='black', alpha=0.5)
                plt.plot(x, df_fit.iloc[i*(columns*rows) + j].fitter(x), c='orange', alpha= 0.6)
                plt.xlim([0, len(x)])
            fig.suptitle("Fits for the cluster no."+str(cluster_number)+" (out of a total of "+str(len(df_fit))+" spectra)")
            fig.supxlabel("Channels")
        ...
        for k in range(0, len(df_fit)-rest*(columns*rows)):
            fig.add_subplot(rows, columns, k+1)
            plt.plot(x, input_data[df_fit.iloc[rest*(columns*rows) + k].Index], c='black', alpha=0.5)
            plt.plot(x, df_fit.iloc[i*(columns*rows) + j].fitter(x), c='orange', alpha= 0.6)
            plt.xlim([0, len(x)])
        fig.suptitle("Fits for the cluster no."+str(cluster_number)+" (out of a total of "+str(len(df_fit))+" spectra)")
        fig.supxlabel("Channels")
...
def visualize_chi2(SOM, df_fit):
    ...
    quant_95, quant_75, quant_50 = np.nanpercentile(df_fit['Chi2'], 95), np.nanpercentile(df_fit['Chi2'], 75), np.nanpercentile(df_fit['Chi2'], 50)
    ...
    plt.text(quant_95, quants[2][2]+170, "95th Percentile", size = 12, alpha =.8)
    
    plt.tick_params(left = False, bottom = False)
    plt.ylabel('# Fits')
    plt.xlabel('Chi2 values')
    plt.title('Distribution of chi2 metric for the '+str(len(df_fit['Chi2']))+' fitted spectra')
...
input_data = [(input_data[i][:]) / (max_intensity)
              for i in range(0, len(input_data))]
...
# 2.3 - visualize the single clusters
# visualize_cluster(SOM, 0, 2)
